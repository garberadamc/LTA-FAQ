...
Download Github repository here: \textcolor{blue}{https://github.com/garberadamc/LTA-FAQ}
...
\textcolor{red}{Note regarding choice of project location:}

If the main project folder is located within too many nested folders it may result in a file-path error
...
plot_t1 <- as.data.frame(output_enum_t1
           [["c4_lca_enum_time1.out"]] 
...
```{r, eval=TRUE}
c_size <- as.data.frame(output_enum_t1
          [["c4_lca_enum_time1.out"]]
...
plot_t1$Var <- factor(plot_t1$Var,
               levels = c("U1","U2","U3","U4","U5"),  # NOTE CHANGE: the number of levels
...
Plot posterior probabilities (7th grade)
```{r, eval=TRUE}
pd_long_t1 %>%
  ...
  scale_colour_grey() + 
  labs(title = "Time 1 LCA Posterior Probability Plot", y = "Probability") +
  theme_cowplot() +
...
# extract posterior probabilities 
plot_t2 <- as.data.frame(output_enum_t2
           [["c4_lca_enum_time2.out"]]
           [["gh5"]][["means_and_variances_data"]]
           [["estimated_probs"]][["values"]]
...
Plot posterior probabilities (10th grade)
```{r, eval=TRUE}
...
pd_long_t2 %>%
  ...
  scale_colour_grey() + 
  labs(title = "Time 2 LCA Posterior Probability Plot", y = "Probability") +
  theme_cowplot() +
  theme(legend.title = element_blank(), 
        legend.position = "top")
```
...
lta_non_inv <- mplusObject(
  ...
  MODEL = 
     "%overall%
      ...
      MODEL c1: !!! the following syntax will estimate item thresholds freely !!!
      %c1#1%
      [AB39M$1-AB39X$1]; 
      %c1#2%
      [AB39M$1-AB39X$1];
...
lta_inv <- mplusObject(
  ...
  MODEL = 
     "%overall%
      ...
      MODEL c1: !!! the following syntax will estimate item thresholds constrained across latent variables !!!
      %c1#1%
      [AB39M$1-AB39X$1] (1-5);  !!! labels that are repeated will constrain parameters equality !!!
      %c1#2%
...
T0 <- lta_models[["X4.class.invariant.out"]][["summaries"]][["ChiSqCategoricalLRT_Value"]]      # test stat. value
T1 <- lta_models[["X4.class.non.invariant.out"]][["summaries"]][["ChiSqCategoricalLRT_Value"]]  # test stat. value
c0 <- lta_models[["X4.class.invariant.out"]][["summaries"]][["LLCorrectionFactor"]]             # scaling factor
c1 <- lta_models[["X4.class.non.invariant.out"]][["summaries"]][["LLCorrectionFactor"]]         # scaling factor
d0 <- lta_models[["X4.class.invariant.out"]][["summaries"]][["ChiSqCategoricalLRT_DF"]]         # degrees of freedom
d1 <- lta_models[["X4.class.non.invariant.out"]][["summaries"]][["ChiSqCategoricalLRT_DF"]]     # degrees of freedom
df <- abs(d0-d1)

# Satorra-Bentler scaled Difference test equations
cd <- (((d0*c0)-(d1*c1))/(d0-d1))
t  <- (((T0*c0)-(T1*c1))/(cd))

t    # Satorra-Bentler chi-square value
df   # difference test degrees of freedom
